...
import { getCurrentUser, isPrincipal as checkIsPrincipal } from "@/server/auth/session";
...
export default async function AcademicsPage() {
  const [{ items }, current, user] = await Promise.all([
    listSessions({ pageSize: 50 }),
    getCurrentSession(),
    getCurrentUser(),
  ]);

  const isPrincipal = checkIsPrincipal(user.role) || user.role === "DEVELOPER";

  return (
    <div>
      <PageHeader
        title="Academic Sessions"
        description="Create, switch, and close academic sessions. Promote students between sessions."
      />
      <AcademicsClient
        sessions={items}
